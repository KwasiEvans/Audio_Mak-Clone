...
function getActivity($activity_id = 0, $fetch = true) {
    ...
    if (IS_LOGGED == true) {
        $music->isSongOwner = ($music->user->id == $getActivity->user_id) ? true : false;
    }
...
function createProPayPalLink() {
    ...
    $item = new Item();
    $item->setName($product)->setQuantity(1)->setPrice($price)->setCurrency($music->config->currency);
    ...
    $amount = new Amount();
    $amount->setCurrency($music->config->currency)->setTotal($total)->setDetails($details);
